...
export const SETTING_DEFINITIONS: SettingDefinition[] = [
  ...
  },
  {
    ...
    description: '鍗曟 LLM 璇锋眰鐨勮秴鏃舵椂闂达紝鍗曚綅姣銆?',
    category: 'llm',
    input: 'number',
  ...
  {
    key: 'LLM_REQUEST_MAX_RETRIES',
    label: 'LLM Request Max Retries',
    description: 'LLM 璇锋眰鍦ㄧ綉缁滄尝鍔ㄦ垨妯″瀷鏈嶅姟涓嶇ǔ瀹氭椂鐨勬渶澶ч噸璇曟鏁般€?',
    category: 'llm',
    input: 'number',
    envKey: 'LLM_REQUEST_MAX_RETRIES',
    defaultValue: '3',
  },
  {
    key: 'LLM_RETRY_BASE_DELAY_MS',
    label: 'LLM Retry Base Delay (ms)',
    description: 'LLM 閲嶈瘯鐨勫熀纭€绛夊緟鏃堕棿锛岀郴缁熶細鍦ㄦ鍩虹涓婂仛鎸囨暟閫€璁┿€?',
    category: 'llm',
    input: 'number',
  ...
  {
    key: 'LLM_REASONING_RETRY_ENABLED',
    label: 'Reasoning Retry Enabled',
    description: '褰?OpenAI 鍏煎妯″瀷鍙繑鍥?reasoning 涓嶈繑鍥?content 鏃讹紝鏄惁鑷姩鍐嶅彂璧疯ˉ璇锋眰銆?',
    category: 'llm',
    input: 'select',
    envKey: 'LLM_REASONING_RETRY_ENABLED',
    defaultValue: '1',
    options: [
      {
        label: '开启',
  ...
  {
    key: 'LLM_ANALYSIS_MAX_TOKENS',
    label: 'Analysis Max Tokens',
    description: '浠撳簱鍒嗘瀽闃舵鐨勬渶澶?Token 涓婇檺銆?',
    category: 'llm',
    input: 'number',
    envKey: 'LLM_ANALYSIS_MAX_TOKENS',
    defaultValue: '1800',
  },
  {
    key: 'LLM_DEEP_READ_MAX_TOKENS',
    label: 'Deep Read Max Tokens',
    description: '鍏抽敭鏂囦欢娣辫闃舵鐨勬渶澶?Token 涓婇檺銆?',
    category: 'llm',
    input: 'number',
    envKey: 'LLM_DEEP_READ_MAX_TOKENS',
    defaultValue: '2200',
  },
  {
    key: 'LLM_CONTENT_MAX_TOKENS',
    label: 'Content Max Tokens',
    description: '涓诲唴瀹圭敓鎴愰樁娈电殑鏈€澶?Token 涓婇檺銆?',
    category: 'llm',
    input: 'number',
    envKey: 'LLM_CONTENT_MAX_TOKENS',
    defaultValue: '3200',
  },
  {
    key: 'LLM_SEO_MAX_TOKENS',
    label: 'SEO Max Tokens',
    description: 'SEO 鏍囬銆佹弿杩般€丗AQ 鐢熸垚闃舵鐨勬渶澶?Token 涓婇檺銆?',
    category: 'llm',
    input: 'number',
    envKey: 'LLM_SEO_MAX_TOKENS',
    defaultValue: '2200',
  },
  {
    key: 'LLM_JSON_REPAIR_MAX_TOKENS',
    label: 'JSON Repair Max Tokens',
    description: 'JSON 淇闃舵鐨勬渶澶?Token 涓婇檺銆?',
    category: 'llm',
    input: 'number',
    envKey: 'LLM_JSON_REPAIR_MAX_TOKENS',
    defaultValue: '3200',
  },
  {
    key: 'LLM_FALLBACK_MAX_TOKENS',
    label: 'Fallback Max Tokens',
    description: '涓绘祦绋嬪け璐ュ悗锛屼繚瀹堢敓鎴愰樁娈电殑鏈€澶?Token 涓婇檺銆?',
    category: 'llm',
    input: 'number',
    envKey: 'LLM_FALLBACK_MAX_TOKENS',
    defaultValue: '2800',
  },
  {
    key: 'LLM_DEEP_READ_FILE_CHAR_LIMIT',
    label: 'Deep Read File Char Limit',
    description: '姣忎釜鍏抽敭鏂囦欢鍦ㄦ繁璇婚樁娈电殑鏈€澶氬瓧绗︽暟锛岀敤浜庢帶鍒?token 鎴愭湰銆?',
    category: 'llm',
    input: 'number',
    envKey: 'LLM_DEEP_READ_FILE_CHAR_LIMIT',
    defaultValue: '3200',
  },
  {
    key: 'LLM_DEEP_READ_TOTAL_CHAR_LIMIT',
    label: 'Deep Read Total Char Limit',
    description: '鍚堝苟鍚庡彂缁欐ā鍨嬬殑娣辫鏂囨湰鏈€澶氬瓧绗︽暟銆?',
    category: 'llm',
    input: 'number',
    envKey: 'LLM_DEEP_READ_TOTAL_CHAR_LIMIT',
    defaultValue: '14000',
  },
  {
...
export function getBooleanSetting(key: string, fallback: boolean) {
  const raw = getSettingValue(key).trim().toLowerCase();
